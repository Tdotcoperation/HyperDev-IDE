...
function fastHash(text=''){let h=2166136261;for(let i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619)}return (h>>>0).toString(16)}
function currentHashes(){const out={};for(const [name,content] of Object.entries(state.files))out[name]=fastHash(String(content));return out}
function changedFileNames(){const hashes=currentHashes();return Object.keys(hashes).filter(name=>state.lastSyncedHashes[name]!==hashes[name])}
function rememberSandboxSync(){state.lastSyncedHashes=currentHashes();localStorage.setItem(`hyperdev-sync-${state.sessionId}`,JSON.stringify(state.lastSyncedHashes))}
